  ...
  <div class="home" :data-stage="stage" :style="{ '--bg-image': bgImage }">
    <div class="loader" aria-hidden="true">
      <div class="loader-core">
        ...
        <div class="loader-status" v-if="stage === 'loading'" :style="{ '--log-fade-ms': `${powFadeMs}ms` }">
          <div class="loader-progress" :data-state="powState" role="progressbar"
            :aria-valuenow="Math.round(powProgress * 100)" aria-valuemin="0" aria-valuemax="100">
            <div class="loader-progress-bar" :style="{ width: `${Math.round(powProgress * 100)}%` }"></div>
          </div>
          <div class="loader-progress-text" :class="{ 'is-hiding': powStatusHiding }">
            <span class="loader-status-text">{{ powStatus }}</span>
            <span v-if="powState === 'working' || powStatusHiding" class="loader-status-spinner" aria-hidden="true">
              <span class="dot"></span>
              <span class="dot"></span>
              <span class="dot"></span>
            </span>
          </div>
          <div class="loader-log" :class="{ 'is-hiding': powLogHiding, 'is-hidden': powLogHidden }">
            <div v-for="(line, index) in powLogs" :key="`pow-log-${index}`" class="loader-log-line">
              {{ line }}
            </div>
          </div>
        </div>
        ...
        <a-row :gutter="[24, 24]" align="middle">
          <a-col :xs="24" :sm="7" :md="5">
            <div class="hero-avatar">
              <a-avatar :size="96" shape="square" src="https://easyimage2.azuneko.com/i/2025/09/14/12lwvxm.webp"
                alt="AMEI avatar" />
            </div>
          </a-col>
